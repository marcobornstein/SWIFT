...
def sync_allreduce(model, rank, size):
    ...
    for param in model.parameters():
        param.data = torch.Tensor(recvdata[param]).cpu()
...
def run(rank, size):
    ...
    for epoch in range(args.epoch):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            # data loading
            # data, target = data.cpu(non_blocking = True), target.cpu(non_blocking = True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--bs', default=100, type=int, help='batch size on each worker')
    ...
    # my addition
    parser.add_argument('--save' , default = 0, type=int, help='save folder')
